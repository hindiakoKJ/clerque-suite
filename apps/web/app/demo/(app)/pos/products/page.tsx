...
export default function DemoProductsPage() {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-6xl">
      ...
      {grouped.map((group) => (
        <div key={group.categoryName} className="bg-white border border-stone-200 rounded-lg overflow-hidden">
          <div className="px-4 py-3 bg-stone-50 border-b border-stone-200">
            <h2 className="font-semibold text-stone-900">{group.categoryName}</h2>
            <p className="text-xs text-stone-500">{group.items.length} item(s)</p>
          </div>
          <table className="w-full">
            <thead className="bg-stone-50/50 border-b border-stone-100 text-[11px]">
            ...
            <tbody className="divide-y divide-stone-100">
              {group.items.map((p) => (
                <tr key={p.id} className="hover:bg-stone-50">
                  ...
                  <td className="px-4 py-2.5 text-stone-600 font-mono text-xs hidden sm:table-cell">{p.sku}</td>
                  <td className="px-4 py-2.5 text-stone-600 font-mono text-xs hidden md:table-cell">
                    {p.barcode ?? '—'}
                  </td>
                  ...
                  <td className="px-4 py-2.5 text-center">
                    {p.isVatable ? (
                      ...
                      <span className="inline-flex px-2 py-0.5 text-[10px] font-semibold rounded bg-stone-100 text-stone-600">
                        Exempt
                      </span>
                    )}
                  </td>
