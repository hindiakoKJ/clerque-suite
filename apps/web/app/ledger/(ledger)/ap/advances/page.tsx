...
interface Bill   { id: string; billNumber: string; balanceAmount: number; status: string }
...
export default function VendorAdvancesPage() {
  ...
  const advancesQ = useQuery<AdvancesResponse>({
    queryKey: ['ap/vendor-advances'],
    queryFn:  () => api.get('/ap/vendor-advances').then((r) => r.data),
  });
  const vendorsQ = useQuery<Vendor[]>({
    queryKey: ['ap/vendors/all'],
    queryFn:  () => api.get('/ap/vendors?pageSize=500').then((r) => r.data?.data ?? r.data ?? []),
  });

  const [showCreate, setShowCreate] = useState(false);
  const [applyTarget, setApplyTarget] = useState<VendorAdvance | null>(null);
  const [refundTarget, setRefundTarget] = useState<VendorAdvance | null>(null);
    ...
    <div className="space-y-6 p-6">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold flex items-center gap-2">
            <Wallet className="w-6 h-6 text-[var(--accent)]" /> Vendor Advances
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            Vendor prepayments / deposits we paid before receiving the bill.
            Sits as an asset until applied to a bill.
          </p>
        </div>
        <button
          ...
          <tbody>
            ...
            {advancesQ.data?.data.map((a) => (
              <tr key={a.id} className="border-t">
                ...
                <td className="px-4 py-2.5">{a.vendor.name}</td>
                ...
                <td className="px-4 py-2.5 text-right whitespace-nowrap">
                  {a.status === 'DRAFT' && (
                    <PostBtn id={a.id} onDone={() => qc.invalidateQueries({ queryKey: ['ap/vendor-advances'] })} />
                  )}
      ...
      {showCreate && (
        <CreateAdvanceModal
          ...
          onCreated={() => {
            setShowCreate(false);
            qc.invalidateQueries({ queryKey: ['ap/vendor-advances'] });
          }}
        />
      )}
      {applyTarget && (
...
function PostBtn({ id, onDone }: { id: string; onDone: () => void }) {
  const m = useMutation({
    mutationFn: () => api.post(`/ap/vendor-advances/${id}/post`, {}, {
      headers: { 'Idempotency-Key': `post-va-${id}-${Date.now()}` },
    }),
...
function CreateAdvanceModal({
  ...
  const m = useMutation({
    mutationFn: () => api.post('/ap/vendor-advances', {
      vendorId, advanceDate, method, totalAmount, reference: reference || undefined, description: description || undefined,
    }),
  ...
  return (
    <Modal title="New Vendor Advance" onClose={onClose}>
      <div className="space-y-3">
        ...
        <Field label="Reference (Check#, GCash ref)">
      ...
      <div className="flex justify-end gap-2 mt-4">
        ...
        <button onClick={() => m.mutate()} disabled={!vendorId || !totalAmount || m.isPending}
...
function ApplyAdvanceModal({
...
}: { advance: VendorAdvance; onClose: () => void; onApplied: () => void }) {
  const billsQ = useQuery<{ data: Bill[] }>({
    queryKey: ['ap/bills/open', advance.vendor.id],
    queryFn:  () => api.get(`/ap/bills?vendorId=${advance.vendor.id}&status=OPEN,PARTIALLY_PAID&pageSize=100`).then((r) => r.data),
  });
  ...
  const m = useMutation({
    mutationFn: () => api.post(`/ap/vendor-advances/${advance.id}/apply`, { billId, amount }),
  ...
  return (
    <Modal title={`Apply ${advance.advanceNumber}`} onClose={onClose}>
          ...
          <select className="w-full border rounded px-2 py-1.5 text-sm bg-background"
            ...
            {bills.map((b) => (
              <option key={b.id} value={b.id}>
                {b.billNumber} — bal {formatPeso(Number(b.balanceAmount))}
              </option>
            ))}
          </select>
        </Field>
      ...
      <div className="flex justify-end gap-2 mt-4">
        <button onClick={onClose} className="px-3 py-1.5 text-sm rounded border hover:bg-muted">Cancel</button>
        <button onClick={() => m.mutate()} disabled={!billId || !amount || m.isPending}
          className="px-3 py-1.5 text-sm rounded bg-[var(--accent)] text-white hover:opacity-90 disabled:opacity-50">
          Apply
        </button>
      </div>
...
}: { advance: VendorAdvance; onClose: () => void; onDone: () => void }) {
  const [method, setMethod] = useState<PaymentMethod>('CASH');
  const [reference, setReference] = useState('');
  const m = useMutation({
    mutationFn: () => api.post(`/ap/vendor-advances/${advance.id}/refund`,
      { method, reference: reference || undefined },
      { headers: { 'Idempotency-Key': `refund-va-${advance.id}-${Date.now()}` } }),
