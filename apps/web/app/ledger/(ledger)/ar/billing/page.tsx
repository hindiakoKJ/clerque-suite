...
interface AgingResponse {
  ...
  bucket90plus:  number;
  total:         number;
  customers:     { id: string; name: string; balance: number; daysPastDue: number }[];
}
...
export default function ARBillingPage() {
  ...
  const { data: list, isLoading } = useQuery<ListResponse>({
    queryKey: ['ar-billing-list', statusFilter, overdueOnly, dueBucket],
    queryFn:  () => api.get(`/ar/invoices?${params.toString()}`).then((r) => r.data),
    enabled:  !!user,
  });

  const { data: aging } = useQuery<AgingResponse>({
    queryKey: ['ar-billing-aging'],
    queryFn:  () => api.get('/ar/invoices/aging').then((r) => r.data),
    enabled:  !!user,
  });
  ...
  function refresh() {
    ...
    qc.invalidateQueries({ queryKey: ['ar-billing-aging'] });
    if (selected) {
