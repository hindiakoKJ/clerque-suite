...
interface LaundryDetail {
  ...
  publicStubToken?: string | null;
  customer:     { id: string; name: string; contactPhone: string | null; address: string | null; loyaltyVisits?: number } | null;
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {order.isDelivery && (
            <div className="pt-2 border-t border-border space-y-1">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="capitalize">{(order.deliveryStatus ?? 'pending').toLowerCase().replace(/_/g, ' ')}</span>
              </div>
        ...
        {/* Digital claim stub — copyable link the cashier sends to the
            customer via SMS / chat. The /stub/<token> page shows live status
            without requiring login. */}
        {order.publicStubToken && (
          <section className="p-4 rounded-xl border border-border bg-card space-y-2 md:col-span-2">
            <h2 className="text-sm font-semibold">Digital claim stub</h2>
            <p className="text-xs text-muted-foreground">
              Send this link to the customer — they'll see live status without logging in.
              The token is unguessable so other customers can't iterate to find it.
            </p>
            <div className="flex items-center gap-2">
              <input
                readOnly
                value={typeof window !== 'undefined' ? `${window.location.origin}/stub/${order.publicStubToken}` : ''}
                onFocus={(e) => e.currentTarget.select()}
                className="flex-1 rounded-lg border border-border bg-background px-3 py-2 text-xs font-mono"
              />
              <button
                onClick={() => {
                  const url = `${window.location.origin}/stub/${order.publicStubToken}`;
        ...
        {order.customer && (order.customer.loyaltyVisits ?? 0) > 0 && (
          <section className="p-4 rounded-xl border border-amber-300/60 bg-amber-50/40 dark:bg-amber-950/20 dark:border-amber-700 space-y-2 md:col-span-2">
            <h2 className="text-sm font-semibold text-amber-800 dark:text-amber-300">Loyalty progress</h2>
            <p className="text-xs text-amber-700 dark:text-amber-300/90">
              {order.customer.name} has completed{' '}
              <strong>{order.customer.loyaltyVisits}</strong> claim{order.customer.loyaltyVisits === 1 ? '' : 's'}.
              {(order.customer.loyaltyVisits ?? 0) % 10 === 9 && ' Next visit unlocks a free wash.'}
            </p>
              ...
              {Array.from({ length: 10 }).map((_, i) => (
                <span
                  ...
                  className={`w-3 h-3 rounded-full ${
                    i < ((order.customer!.loyaltyVisits ?? 0) % 10)
                      ? 'bg-amber-500'
